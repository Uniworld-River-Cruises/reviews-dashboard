  ...
  if (!itinerary) {
    return (
      <div className="text-center py-24">
        ...
        <Link href="/itineraries" className="text-[#C5A258] hover:underline">
          Back to Itineraries
        </Link>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KpiCard
          ...
          delta={`${ratingDelta >= 0 ? "+" : ""}${ratingDelta.toFixed(2)} vs fleet avg`}
        />
        <KpiCard
          title="Total Reviews"
          value={itinerary.reviewCount.toLocaleString()}
        />
        <KpiCard
          title="5-Star %"
          value={`${itinerary.fiveStarPercent.toFixed(1)}%`}
          trendUp={fiveStarDelta >= 0}
          delta={`${fiveStarDelta >= 0 ? "+" : ""}${fiveStarDelta.toFixed(1)}% vs fleet avg`}
        />
        <KpiCard
          title="4+ Star %"
          value={`${itinerary.fourPlusPercent.toFixed(1)}%`}
        />
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        ...
        <div className="flex flex-wrap gap-3">
          {itinerary.ships.map((ship) => (
            <Link
              key={ship}
              href={`/ships/${ship.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "")}`}
              className="inline-flex items-center gap-2 bg-gray-50 rounded-lg px-4 py-3 hover:bg-gray-100 transition-colors"
            >
              <svg className="h-5 w-5 text-[#1B3A5C]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
