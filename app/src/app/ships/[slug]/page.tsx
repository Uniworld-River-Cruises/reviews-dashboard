  ...
  if (!ship) {
    return (
      <div className="text-center py-24">
        ...
        <Link href="/ships" className="text-[#C5A258] hover:underline">
          Back to Ships
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <KpiCard
          title="Total Reviews"
          value={ship.reviewCount.toLocaleString()}
        />
        <KpiCard
          title="5-Star %"
          value={`${ship.fiveStarPercent.toFixed(1)}%`}
          trendUp={fiveStarDelta >= 0}
          delta={`${fiveStarDelta >= 0 ? "+" : ""}${fiveStarDelta.toFixed(1)}% vs fleet avg`}
        />
        <KpiCard
          title="4+ Star %"
          value={`${ship.fourPlusPercent.toFixed(1)}%`}
        />
      </div>
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-2 mb-3">
                <div>
                  ...
                  <p className="text-sm font-semibold text-[#1B3A5C]">{it.averageRating.toFixed(2)}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-400">Reviews</p>
                  <p className="text-sm font-semibold text-[#1B3A5C]">{it.reviewCount}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-400">5-Star</p>
                  <p className="text-sm font-semibold text-[#1B3A5C]">{it.fiveStarPercent.toFixed(1)}%</p>
                </div>
              </div>
